...
``--check`` writes nothing. data.js carries a ``digest`` of the board as
rendered; --check recomputes it from the model set the latest results imply and
exits 1 when they differ — a figure has moved, or a model was added or removed
...
EDITORIAL_MODEL_FIELDS = (
    "id", "name", "hex", "panel", "added", "addedOrder",
    "pNote", "rNote", "cNote", "sNote",
)
...
def render_data_js(models: list[dict[str, Any]], cats: list[dict[str, Any]],
                   source_run: str, generated: str, digest: str) -> str:
    ...
    for m in models:
        fields = [(k, m.get(k)) for k in (
            "id", "name", "hex", "panel", "added", "addedOrder", "tested",
        )]
...
def board_digest(rows: dict[str, dict[str, Any]]) -> str:
    """A hash over every derived value, keyed by model, with the roster as the
    set of keys.

    data.js stores the digest of the board as rendered; the watch workflow
    recomputes it from the model set the latest results imply. The two differ
    when a figure has moved or when a model has been added or removed upstream —
    one comparison for both kinds of drift. Editorial fields (colour, panel,
    footnotes) are not derived from the results and are excluded, so re-deciding
    one never trips the check.
    """
    fields = ("tested", "pass", "lo", "hi", "rubric",
              "cost", "ttft", "costTask", "cv", "dv", "rv", "sv", "tv")
    canon = {name: [row.get(f) for f in fields] for name, row in sorted(rows.items())}
...
def main() -> int:
    ...
    addition_order = {
        model["name"]: (model.get("added", "9999-12-31"), model.get("addedOrder", 9999))
        for model in merged
    }
    cats.sort(key=lambda c: addition_order[c["name"]])

    latest = max(r["run_id"] for r in summary["records"])

    # The digest that goes into data.js describes the board as rendered: the
    # roster is the set of models actually on it, and their values are the ones
    # being shown. Checking recomputes the digest over the full set the results
    # imply, so a model present upstream but not yet drawn (it still needs a
    # colour and a panel decision) shows up as a roster difference.
    by_name = {m["name"]: m for m in existing}
    board_state = {m["name"]: derived.get(m["name"]) or by_name[m["name"]] for m in merged}
    board_hash = board_digest(board_state)
    results_hash = board_digest(derived)

    rendered = render_data_js(merged, cats, latest, summary["generated_utc"], board_hash)

    if args.report:
        args.report.write_text(report, encoding="utf-8")

    if args.check:
        # One comparison covers every kind of drift a person must act on: a moved
        # figure and a model added or removed upstream both change the digest. A
        # data.js with no digest predates this check and is treated as drift so
        # the next regeneration establishes the baseline.
        stored = re.search(r"digest:'([^']*)'", existing_text)
        if stored is None or stored.group(1) != results_hash:
            print(report)
